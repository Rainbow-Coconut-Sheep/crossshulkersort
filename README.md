...
## Versions / 版本分支（39 个，1.15–26.2 全覆盖）

整理核心算法全分支一致；差异只在存取层（组件/NBT）、网络包、按钮/mixin 胶水。

| 分支 | MC | 存档 | Java | IS 跟随 | 验证 |
|---|---|---|---|---|---|
| `main` | 26.2 | 组件 | 25 | ✅ 0.32.2 | 回归19+fuzz5000+实机 |
| `mc/26.1` | 26.1–26.1.2 | 组件 | 25 | ✅ 0.31.6 | 回归+fuzz+冒烟 |
| `mc/1.21.11` | 1.21.11 | 组件 | 21 | ➖（恒 ID 序） | 冒烟 |
| `mc/1.21.10` | 1.21.10 | 组件 | 21 | ➖ | 冒烟 |
| `mc/1.21.9` | 1.21.9 | 组件 | 21 | ➖ | 编译 |
| `mc/1.21.8`–`mc/1.21` | 1.21–1.21.8 | 组件 | 21 | ✅ 0.24.60（1.21/1.21.1）/➖ | 编译（1.21.1 冒烟） |
| `mc/1.20.6` | 1.20.6 | 组件 | 21 | ➖ | 编译 |
| `mc/1.20.5` | 1.20.5 | 组件 | 21 | ➖ | 冒烟 |
| `mc/1.20.4`–`mc/1.20` | 1.20–1.20.4 | NBT | 17 | ➖ | 编译（1.20.1 冒烟） |
| `mc/1.19.4` | 1.19.4 | NBT | 17 | ➖ | 冒烟 |
| `mc/1.19.3`–`mc/1.19` | 1.19–1.19.3 | NBT | 17 | ➖ | 编译 |
| `mc/1.18.2` | 1.18.2 | NBT | 17 | ➖ | 冒烟 |
| `mc/1.18.1`–`mc/1.18` | 1.18–1.18.1 | NBT | 17 | ➖ | 编译 |
| `mc/1.17.1` | 1.17.1 | NBT | 17 | ➖ | 冒烟 |
| `mc/1.17` | 1.17 | NBT | 17 | ➖ | 冒烟 |
| `mc/1.16.5`–`mc/1.16` | 1.16–1.16.5 | NBT | 8 | ➖ | 回归19+fuzz5000（1.16.5/1.16.4/1.16.3/1.16.2/1.16.1/1.16 全跑过）+冒烟 |
| `mc/1.15.2` | 1.15.2 | NBT | 8 | ➖ | 回归19+fuzz5000+冒烟 |
| `mc/1.15.1` | 1.15.1 | NBT | 8 | ➖ | 回归19+fuzz5000+冒烟 |
| `mc/1.15` | 1.15 | NBT | 8 | ➖ | 回归19+fuzz5000 |

说明：
- IS 跟随仅在 `main`/`mc/26.1`/`mc/1.21.1`（及同 API 的 1.21 小版本编译依赖）启用；其余分支恒为原版 ID 序（`useItemScrollerOrder` 开关保留但无实质区别）。
- 新 Yarn（≥1.17 时代映射）分支的回归/fuzz 无法独立运行（`SimpleRegistry` 跨包访问是自 1.17 起的已知 Yarn 限制），逻辑门在 `main` 与 1.15/1.16 分支执行——三处全绿且算法同源。
- 冒烟 = 无头服务端启动 Done 且 mod 零错误加载；编译 = `clean build` 通过。实机 Q 测试目前仅 26.2，欢迎各版本反馈。
